 ...
 */
router.get("/", authorize([Role.admin]), validate(GetStudentQuerySchema), asyncHandler(async (req, res) => {
    const result = await studentManageService.getStudentslist(req.query);
    res.status(200).json({
        success: true,
...
router.get("/totalStudent", asyncHandler(async (req, res) => {
 ...
 */
router.get("/:id", authorize([Role.admin]), validate(StudentIdParamSchema), asyncHandler(async (req, res) => {
