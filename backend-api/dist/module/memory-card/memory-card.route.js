...
import { StartMemoryGameSchema, SaveProgressSchema, FinishMemoryGameSchema, } from "./memory-card.schema.js";
...
router.get("/start", Authenticate, validate(StartMemoryGameSchema), asyncHandler(async (req, res) => {
    const userId = Number(req.user?.userId);
    const result = await memoryCardService.startGame(userId, req.query);
    res.status(200).json({
        success: true,
...
router.post("/progress", Authenticate, validate(SaveProgressSchema), asyncHandler(async (req, res) => {
    const userId = Number(req.user?.userId);
...
router.post("/finish", Authenticate, validate(FinishMemoryGameSchema), asyncHandler(async (req, res) => {
    const userId = Number(req.user?.userId);
