...
router.get("/start", Authenticate, validate(StartGameSchema), async (req, res, next) => {
    try {
        const userId = Number(req.user?.userId);
        const result = await gameService.startGame(userId, req.query);
...
router.post("/match", Authenticate, validate(MatchPairSchema), async (req, res, next) => {
    try {
        const userId = Number(req.user?.userId);
...
router.post("/finish", Authenticate, validate(FinishGameSchema), async (req, res, next) => {
    try {
        const userId = Number(req.user?.userId);
